...
const SideToggleGroup = ({ isOpen, isMobile, groupId, groupName }) => {
  ...
  return (
    <>
      <aside className={`sidebar ${isOpen ? "open" : ""} ${isMobile ? "mobile" : ""}`}>
        <div className="sidebar-content">
          ...
          <nav className="sidebar-nav">
            <button className="nav-button" onClick={()=>setShowAddMemberModal(true)}>
              <FaPlus />
              <span>Add Member</span>
            </button>
            <button className="nav-button" onClick={()=> setShowRemoveMemberModal(true)}>
              <FaUserMinus />
              <span>Remove Member</span>
            </button>
            <button className="nav-button danger" onClick={()=> setShowDeleteGroupModal(true)}>
              <FaTrash />
      ...
      {
        showAddMemberModal && <AddGroupMembersModal onClose={()=> setShowAddMemberModal(false)} groupId={groupId}/>
      }
      
      {
        showRemoveMemberModal && <RemoveGroupMembersModal onClose={()=> setShowRemoveMemberModal(false)} groupId={groupId}/>
      }
